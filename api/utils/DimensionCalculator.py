...
def unpackRect(tl, tr, br, bl):
	topleft = vars(Point(float(tl[0]), float(tl[1])))
	topright = vars(Point(float(tr[0]), float(tr[1])))
	botright = vars(Point(float(br[0]), float(br[1])))
	botleft = vars(Point(float(bl[0]), float(bl[1])))
	return {
		"topleft": topleft,
		"topright": topright,
		"botright": botright,
...
def calculateDims(imagePath, objHeight):
	...
	print(crop)
	result = {"crop": crop, "width": dimA, "length": dimB, "type": type}
	return result
